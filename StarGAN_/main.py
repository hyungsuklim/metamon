...
def main(config):
    ...
    if config.dataset in ['image']:
        image_loader = get_loader(config.input_image_path, config.metadata_path, config.celebA_crop_size,
                                  config.image_size, config.batch_size, 'image', config.mode)

    # Solver
    solver = Solver(celebA_loader, rafd_loader, image_loader, config)

    if config.mode == 'train':
        if config.dataset in ['CelebA', 'RaFD', 'image']:
            solver.train()
...
if __name__ == '__main__':
    ...
    # Training settings
    parser.add_argument('--dataset', type=str, default='CelebA', choices=['CelebA', 'RaFD', 'image', 'Both'])
